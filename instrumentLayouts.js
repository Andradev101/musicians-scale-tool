function resetChildrenElements() {
    ...
    }//remove every children except the fret counter
...
function gtrLayout() {
    ...
    for (let i = 0; i < strings.length ; i++) {
        ...
        for (let j = rootNote; j <= rootNote+22; j++) {//e.g starts from 4 and it goes until (4+22) 26
    ...
    for (let counter = 0; counter < frets.length; counter++) {
...
function pianoLayout(){
    ...
    for (let j = rootNote; j <= rootNote+12; j++) {
        const noteBox = document.createElement("div");
        const noteName = document.createElement("p");
        stringContainer.append(noteBox)
        noteBox.append(noteName)
    ...
    for (let counter = 0; counter < (frets.length/2) +1; counter++) {
        fretNumber = document.createElement("p");
        fretNumber.innerText = counter;
        fretCounting.append(fretNumber) 
